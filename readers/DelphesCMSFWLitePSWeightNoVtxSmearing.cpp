                  ...
                  std::vector<size_t> &vectorPSWeightsIndex)
{
  ...
  double pv_x = 0, pv_y = 0, pv_z = 0;
  double no_smear_limit = 1e-8; // https://github.com/cms-sw/cmssw/blob/master/Configuration/StandardSequences/python/VtxSmearedNoSmear_cff.py
  ...
  for(itParticle = handleParticle->begin(); itParticle != handleParticle->end(); ++itParticle)
  {
    ...
    candidate->Position.SetXYZT((x-pv_x) * 10.0, (y-pv_y) * 10.0, (z-pv_z) * 10.0, 0.0);
